import puppeteer, { Browser, Page, Target, BrowserContext, Protocol, CDPSession } from "puppeteer-core";
...
export class CDPService extends EventEmitter {
  ...
  private tracingSession: CDPSession | null;
  ...
  public async startTracing(sessionId: string): Promise<void> {
    if (!this.primaryPage) {
    ...
    // Start tracing
    await client.send('Tracing.start' as any, {
      ...
      options: 'sampling-frequency=30'
    });

    this.tracingSession = client;
    this.videoPath = `videos/${sessionId}.webm`;
  }

  public async stopTracing(): Promise<string | null> {
    if (!this.tracingSession || !this.videoPath) {
      return null;
    }

    try {
      // Stop tracing
      await this.tracingSession.send('Tracing.end' as any);
      
      // Get the trace data
      const result = await this.tracingSession.send('Tracing.getTraceData' as any) as { streamHandle: string };
      const streamHandle = result.streamHandle;
      
      // Process the stream and save as video
      const writeStream = fs.createWriteStream(this.videoPath);
      
      while (true) {
        const { data, eof } = await this.tracingSession.send('IO.read' as any, { handle: streamHandle }) as { data?: string; eof: boolean };
      ...
      await this.tracingSession.send('IO.close' as any, { handle: streamHandle });
      ...
      this.logger.error(`Error stopping tracing: ${error}`);
      return null;
    }
  }
