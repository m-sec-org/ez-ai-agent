    ...
    def conduct_tool(*agents: Agent) -> Callable:
        ...
        def create_conduct_tool(agents: List[Any]) -> Callable:
            ...
            async def conduct_tool(
                ...
                print(f"[EZ AI Agent] 启动 Agentic 渗透测试，计划执行 {len(tasks)} 项任务.")
                ...
                for instruction_item in tasks:
                    ...
                    print(f"[EZ AI Agent] '{task.task_id}'：'{task.instruction}'")
                    print(
                        f"[EZ AI Agent] 调度 '{task.agent_id}' Agent 执行 '{task.task_id}' 任务。"
                    )

                    if not target_agent:
                        print(
                            f"[EZ AI Agent] Warning: Agent {task.agent_id} not found. Available agents: {list(agent_map.keys())}"
                        )
                        continue
                    ...
                    if agent_call_counts[task.agent_id] > MAX_AGENT_ITERATIONS:
                        print(
                            f"[EZ AI Agent] Warning: Agent {task.agent_id} exceeded maximum iterations"
                        )
                        continue
                    ...
                    print(f"\n[EZ AI Agent] 启动 {task.agent_id} Agent 执行任务。")
